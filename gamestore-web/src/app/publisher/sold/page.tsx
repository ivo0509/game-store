...
import { getPublisherSoldGames } from "@/services/publisherService";
...
export default async function PublisherSoldGamesPage() {
  const session = await requirePublisher();
  const soldGames = await getPublisherSoldGames(session.userId);
      ...
      {soldGames.length === 0 ? (
        <div className="text-center py-24 bg-white rounded-lg border border-gray-200">
          ...
          <p className="text-gray-500">
            Your sold games will appear here once users purchase them.
          </p>
        </div>
      ) : (
